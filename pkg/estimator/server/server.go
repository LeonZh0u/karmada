...
func NewEstimatorServer(
	...
	estimateFramework, err := frameworkruntime.NewFramework(registry,
		...
		frameworkruntime.WithDefaultSortingCriterias(),
	)
	if err != nil {
		return es, err
	}
...
func (es *AccurateSchedulerEstimatorServer) GetVictimResourceBindings(ctx context.Context, request *pb.PreemptionRequest) (response *pb.PreemptionResponse, rerr error) {
	...
	defer func(start time.Time) {
		...
		klog.V(2).Infof("Finish calculating cluster victim resourebindings to preempt for resource(%s), victim resourcebindings: %s, time elapsed: %s", object, response.VictimResourceBindings, time.Since(start))
	}(time.Now())
	...
	if len(resourcebindingObjRef) == 0 {
		return nil, errors.New("no preemptable victims found for incoming resourcebinding")
	}
